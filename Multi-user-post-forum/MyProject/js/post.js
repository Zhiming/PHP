window.onload = function () {
	...
	qa[0].onclick = function(){
		window.open('q.php?num=48&path=qpic\\1\\','q','width=400,height=400,scrollbar=auto');
	};
	qa[1].onclick = function(){
		window.open('q.php?num=10&path=qpic\\2\\','q','width=400,height=400,scrollbar=auto');
	};
	qa[2].onclick = function(){
		window.open('q.php?num=39&path=qpic\\3\\','q','width=400,height=400,scrollbar=auto');
	};
	//clicking some area other than the font size would close
	//the font size window
	...
	ubbimg[8].onclick = function () {
		var url = prompt('Please enter a website:','http://');
		if (url) {
			if (/^https?:\/\/(\w+\.)?[\w\-\.]+(\.\w+)+$/.test(url)) {
				content('[url]'+url+'[/url]');
			} else {
				alert('Illegal website address!');
			}
		}
	};
	...
	ubbimg[10].onclick = function () {
		var img = prompt('Please enter an picture address:','');
		if(img){
			content('[img]'+img+'[/img]');
		}
	};
	ubbimg[11].onclick = function () {
		var flash = prompt('Please enter the address of a flash:','http://');
		if (flash) {
			if (/^https?:\/\/(\w+\.)?[\w\-\.]+(\.\w+)+/.test(flash)) {
				content('[flash]'+flash+'[/flash]');
			} else {
				alert('Illegal flash address!');
			}
		}
	};
